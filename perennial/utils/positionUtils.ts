...
export function utilization(pre: PrePosition, pos: Position) {
  ...
  if (nextPosition.maker === Number(0n)) return Big6Math.ONE;
...
export function socialization(pre: PrePosition, pos: Position) {
  const nextPosition = next(pre, pos);
  if (nextPosition.taker === Number(0n)) return Big6Math.ONE;
...
export const calcSkew = (marketSnapshot?: MarketSnapshot) => {
  ...
  const longSkew = skew > 0n ? skew : Big6Math.ONE - Big6Math.abs(skew);
  const shortSkew = skew < 0n ? Big6Math.abs(skew) : Big6Math.ONE - skew;
  return {
    skew,
    longSkew,
    shortSkew,
...
export function calcInterfaceFee({
  ...
  if (!marketSnapshot || !positionDelta || !feeInfo) {
    return {
      interfaceFeeBps: feeInfo?.feeAmount ?? 0n,
      interfaceFee: 0n,
    };
  }
  const {
